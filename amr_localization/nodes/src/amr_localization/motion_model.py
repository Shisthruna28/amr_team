...
class MotionModel:
    ...
    def sample(self, pose):
        ...
        if self.forward != 0.0 and self.lateral != 0.0:
            #tranforming the motion model to global frame and updating it.
            pose.x =pose.x+ (random.gauss(self.forward, self.sigtran))*math.cos(pose.theta) - (random.gauss(self.lateral, self.sigtran))*math.sin(pose.theta)
            pose.y =pose.y+(random.gauss(self.forward, self.sigtran))*math.sin(pose.theta) + (random.gauss(self.lateral, self.sigtran))*math.cos(pose.theta)
